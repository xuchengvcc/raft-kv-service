...
func main() {
	var server_or_client string

	flag.StringVar(&server_or_client, "s", "server", "启动 service: server, 启动 client: client")
	flag.Parse()
	...
	case "client":
		StartClient()
	default:
		mylog.DPrintln("Not Support ", server_or_client, ", only support [server or client]")
	}
}
