...
type Raftserver struct {
	...
	addr      string                    // IP
	peerAddrs map[int32]string          // other Node IP
	peers     map[int32]rrpc.RaftClient // RPC end points of all peers
	persister *persister.Persister      // Object to hold this peer's persisted state
	me        int32                     // this peer's index into peers[]
	dead      int32                     // set by Kill()
	leaderIdx int32                     // leader's index
	...
	nextIndex  map[int32]int64 // 用全局索引
	matchIndex map[int32]int64 // 用全局索引
...
func (rf *Raftserver) QuickApply() {
	...
	mylog.DPrintf("%v Start quickapply, lastApply: %v, commitIdx: %v", rf.me, rf.lastApplied, rf.commitIndex)
...
func (rf *Raftserver) CommitCheck() {
	for !rf.killed() {
		// mylog.DPrintf("%v %v CommitCheck Try Get Lock", roleName(rf.role), rf.me)
		rf.mu.Lock()
		// mylog.DPrintf("%v %v CommitCheck Get the Lock", roleName(rf.role), rf.me)
		// 使用缓存，不然导致死锁
		...
		for rf.commitIndex > tmpApplied {
			...
			mylog.DPrintf("%v %v add Command %v(Idx: %v) to Buffer", roleName(rf.role), rf.me, message.Command, message.CommandIndex)
		}
		rf.mu.Unlock()
		...
		for _, msg := range buffer {
			...
			rf.mu.Unlock()
			// mylog.DPrintf("%v %v CommitCheck UnLock", roleName(rf.role), rf.me)
			rf.applyCh <- msg
			// mylog.DPrintf("%v %v CommitCheck try Get Lock", roleName(rf.role), rf.me)
			rf.mu.Lock()
			// mylog.DPrintf("%v %v Commited Command %v(Idx: %v) from Buffer", roleName(rf.role), rf.me, msg.Command, msg.CommandIndex)
...
func (rf *Raftserver) persist() {
	...
	// raftstate := w.Bytes()
	// mylog.DPrintf("%v %v persist()", roleName(rf.role), rf.me)

	state := &persister.State{VotedFor: rf.votedFor, CurrentTerm: rf.currentTerm}
	// mylog.DPrintf("%v 当前logs: %v", rf.me, rf.log)
	if len(rf.log) > 0 {

		logs := make([]*rrpc.Entry, len(rf.log)-1)
		// mylog.DPrintf("rf.logs: %v", rf.log)
...
func (rf *Raftserver) readPersist(logs []*rrpc.Entry, voteFor int32, currentTerm int64, commitIndex int64) {
	mylog.DPrintf("%v CommitIndex: %v", rf.me, commitIndex)
	rf.votedFor = voteFor
	rf.currentTerm = currentTerm
	rf.commitIndex = commitIndex
	if len(logs) < 1 {
		mylog.DPrintf("%v readLogs is empty", rf.me)
		return
	}
	rf.log = logs
}
...
func (rf *Raftserver) readSnapshot(data []byte, lastIndex int64, lastTerm int64) {
	if len(data) < 1 {
		mylog.DPrintf("%v readSnapShot is empty", rf.me)
		return
	}
	rf.snapShot = data
	...
	mylog.DPrintf("%v readSnapShot Success", rf.me)
}
...
func (rf *Raftserver) Snapshot(index int64, snapshot []byte) {
	...
	if rf.commitIndex < index || index <= rf.lastIncludedIndex {
		mylog.DPrintf("%v %v(ComitIdx: %v,LastIncludIdx: %v) Reject Snapshot, Idx %v", roleName(rf.role), rf.me, rf.commitIndex, rf.lastIncludedIndex, index)
		return
	}
	// 2. 将 Snapshot 保存，Follower 可能需要 Snapshot，持久化需要 Snapshot 保存
	rf.snapShot = snapshot
	// mylog.DPrintf("%v %v(ComitIdx: %v,LastIncludIdx: %v) Receive Snapshot, Idx %v, Local: %v, LogLen: %v", roleName(rf.role), rf.me, rf.commitIndex, rf.lastIncludedIndex, index, rf.GlobalToLocal(index), len(rf.log))
	rf.lastIncludedTerm = rf.log[rf.GlobalToLocal(index)].Term
	// 3. 截断 log
	// mylog.DPrintf("%v %v(ComitIdx: %v,LastIncludIdx: %v) Start Snapshot, Idx: %v,BeforeCut: %v", roleName(rf.role), rf.me, rf.commitIndex, rf.lastIncludedIndex, index, len(rf.log))
	rf.log = rf.log[rf.GlobalToLocal(index):]
	rf.lastIncludedIndex = index
	mylog.DPrintf("%v lastIncludedIndex update to %v", rf.me, index)
	if rf.lastApplied < index {
		rf.lastApplied = index
	}
	// 4. 调用 persist
	// mylog.DPrintf("%v %v(ComitIdx: %v,LastIncludIdx: %v) Start Snapshot, Idx: %v,LogCutTo: %v", roleName(rf.role), rf.me, rf.commitIndex, rf.lastIncludedIndex, index, len(rf.log))
	rf.persist()
}
...
func (rf *Raftserver) RequestVote(ctx context.Context, args *rrpc.RequestVoteArgs) (*rrpc.RequestVoteReply, error) {
	...
	// mylog.DPrintf("%v(V: %v) << %v's requestVote", rf.me, rf.votedFor, args.CandidateId)
	reply := &rrpc.RequestVoteReply{}
	rf.mu.Lock()
	mylog.DPrintf("%v %v(T: %v, V: %v) <<< C %v(T: %v), Try Lock", roleName(rf.role), rf.me, rf.currentTerm, rf.votedFor, args.CandidateId, args.Term)
	defer rf.mu.Unlock()
	// mylog.DPrintf("%v %v(T: %v, V: %v) <<< C %v(T: %v), Get Lock", roleName(rf.role), rf.me, rf.currentTerm, rf.votedFor, args.CandidateId, args.Term)
	if args.Term < rf.currentTerm {
		// 1. Candidate 的任期小于 Follower 任期
		reply.Term = rf.currentTerm
		reply.VoteGranted = false
		// mylog.DPrintf("%v %v(T: %v, LastLogT: %v, V: %v)  X  C %v(T: %v,LastLogT: %v) Term\n", roleName(rf.role), rf.me, rf.currentTerm, rf.log[len(rf.log)-1].Term, rf.votedFor, args.CandidateId, args.Term, args.LastLogTerm)
		return reply, nil
	}

	if args.Term > rf.currentTerm {
		// 新一轮投票，需要取消上一轮的投票
		// mylog.DPrintf("请求(T: %v)的Term大于自己的Term(T: %v), %v %v自己降级为Follower", args.Term, rf.currentTerm, roleName(rf.role), rf.me)
		rf.votedFor = -1
...
func (rf *Raftserver) SendCommitIndex(ch chan int64) error {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	args := &rrpc.GetCommitIndexArgs{}
	defer cancel()

	resCh := make(chan *rrpc.GetCommitIndexReply, 1)
	...
	go func() {
		...
		r, err := rf.peers[rf.leaderIdx].GetCommitIndex(ctx, args)
		if err != nil {
			mylog.DPrintf("F %d try get CommitIndex from %d, Err: %v", rf.me, rf.leaderIdx, err)
			errCh <- err
			return
		}
		resCh <- r
...
func (rf *Raftserver) AppendEntries(ctx context.Context, args *rrpc.AppendEntriesArgs) (*rrpc.AppendEntriesReply, error) {
	...
	if args.Term > rf.currentTerm {
		...
		rf.votedFor = -1           // 新Leader已经产生或已开启了新选举，消除之前的投票记录
		rf.persist()
	...
	if args.PrevLogIndex >= rf.LocalToGlobal(int64(len(rf.log))) {
		...
		isConflict = true
		mylog.DPrintf("F %v(T: %v,LastLog: %v) doesn't contain L %v(T: %v,FirLog: %v)", rf.me, rf.currentTerm, rf.LocalToGlobal(int64(len(rf.log)-1)), args.LeaderId, args.Term, args.PrevLogIndex+1)
	} else if args.PrevLogIndex < rf.lastIncludedIndex {
		// PrevLogIndex 位置在 Follower 的快照中，快照部分不动，log部分的Term无法验证，因此重新复制log
		reply.XTerm = -1
		reply.XLen = rf.lastIncludedIndex + 1
		isConflict = true
		mylog.DPrintf("F %v(T: %v,LastIncludedLog: %v) Snapshot Contains L %v(T: %v,FirLog: %v)", rf.me, rf.currentTerm, rf.lastIncludedIndex, args.LeaderId, args.Term, args.PrevLogIndex+1)
		...
		isConflict = true
		mylog.DPrintf("F %v(T: %v,LastLogT: %v,LastI: %v,LastIncludIdx: %v) Conflict L %v(T: %v,PreLogT: %v,RreI: %v)", rf.me, rf.currentTerm, rf.log[rf.GlobalToLocal(args.PrevLogIndex)].Term, rf.LocalToGlobal(int64(len(rf.log)-1)), rf.lastIncludedIndex, args.LeaderId, args.Term, args.PrevLogTerm, args.PrevLogIndex)
	}
	...
	if args.LeaderCommit > rf.commitIndex {
		...
		mylog.DPrintf("F %v update commitIndex to %v", rf.me, rf.commitIndex)
		rf.condApply.Signal()
	}
	return reply, nil
}

func (rf *Raftserver) sendAppendEntries(server int32, args *rrpc.AppendEntriesArgs) (*rrpc.AppendEntriesReply, bool) {

	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := rf.peers[server].AppendEntries(ctx, args)
	if err != nil {
		mylog.DPrintf("L %v Append to F %v Err: %v", rf.me, server, err)
		return r, false
	}
	return r, true
...
func (rf *Raftserver) sendAppendEntriesToServer(server int32, args *rrpc.AppendEntriesArgs) {
	// mylog.DPrintf("L %v sendAppendEntriesToServer try get Lock %v", rf.me, server)
	rf.mu.Lock()
	if rf.role != Leader {
		rf.mu.Unlock()
		return
	...
	if reply.Success {
		// mylog.DPrintf("%v %v Update %v matchIdx %v > %v, nextIdx %v > %v", roleName(rf.role), rf.me, server, rf.matchIndex[server], max(rf.matchIndex[server], args.PrevLogIndex+int64(len(args.Entries))), rf.nextIndex[server], args.PrevLogIndex+int64(len(args.Entries)+1))
		...
		for commitLastLog > rf.commitIndex {
			...
			// mylog.DPrintf("ComiLastLog: %v, matchIdx: %v,count: %v", commitLastLog, rf.matchIndex[server], count)
			if count > len(rf.peers)/2 {
				mylog.DPrintf("L %v Commit Log(T:%v, Idx:%v)", rf.me, rf.log[rf.GlobalToLocal(commitLastLog)].Term, commitLastLog)
	...
	if reply.Term > rf.currentTerm {
		mylog.DPrintf("Old Leader %v(T: %v,LastLogI: %v,LastLogT: %v) Received Reply(T: %v), Convert to Follower", rf.me, rf.currentTerm, rf.LocalToGlobal(int64(len(rf.log)-1)), rf.log[len(rf.log)-1].Term, reply.Term)
		...
		mylog.DPrintf("%v %v(LastIncludIdx %v) Conflict %v Reply(T %v,XT %v,XL %v,XI %v)", roleName(rf.role), rf.me, rf.lastIncludedIndex, server, reply.Term, reply.XTerm, reply.XLen, reply.XIndex)
		if reply.XTerm == -1 {
			...
			mylog.DPrintf("%v %v(LastIncludIdx: %v) Reset %v(NextIdx >>> %v, Xlen: %v), ", roleName(rf.role), rf.me, rf.lastIncludedIndex, server, rf.nextIndex[server], reply.XLen)
			return
		}
...
func (rf *Raftserver) InstallSnapshot(ctx context.Context, args *rrpc.InstallSnapshotArgs) (*rrpc.InstallSnapshotReply, error) {
	...
	rf.mu.Lock()
	mylog.DPrintf("Snapshot %v %v(LastIncludedIdx: %v) <<< L %v(LastIncludedIdx: %v)", roleName(rf.role), rf.me, rf.lastIncludedIndex, args.LeaderId, args.LastIncludedIndex)
	defer func() {
		rf.heartbeatTimeStamp = time.Now()
		// rf.ResetHeartTimer(HeartbeatTimeThreshold())
		// mylog.DPrintf("Snapshot %v %v(LastLogI: %v) <<< L %v(LastIncludedIdx: %v) End Reset HeartbeatTime", roleName(rf.role), rf.me, rf.LocalToGlobal(len(rf.log)-1), args.LeaderId, args.LastIncludedIndex)
		rf.mu.Unlock()
	}()
	if args.Term < rf.currentTerm {
		reply.Term = rf.currentTerm
	...
	if hasLogInSnapshot {
		// mylog.DPrintf("%v %v has Log in Snapshot Idx: %v", roleName(rf.role), rf.me, rIdx)
		rf.log = rf.log[rIdx:]
	} else {
		// mylog.DPrintf("%v %v don't has Log in Snapshot, Clear Log", roleName(rf.role), rf.me)
...
func (rf *Raftserver) sendInstallSnapshot(server int32, args *rrpc.InstallSnapshotArgs) (*rrpc.InstallSnapshotReply, bool) {

	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := rf.peers[server].InstallSnapshot(ctx, args)
	if err != nil {
		mylog.DPrintf("L %v Snapshot to F %v Err: %v", rf.me, server, err)
		return r, false
	}
	return r, true
}

func (rf *Raftserver) sendInstallSnapshotToServer(server int32) {
	// mylog.DPrintf("%v %v Preparing SendInstallSnapshotTo %v", roleName(rf.role), rf.me, server)
	rf.mu.Lock()
	if rf.role != Leader {
		rf.mu.Unlock()
		return
	...
	mylog.DPrintf("%v %v SendInstallSnapshotTo %v", roleName(rf.role), rf.me, server)
	rf.mu.Unlock()
	...
	mylog.DPrintf("%v %v Reset Server %v NextIdx %v >>> %v", roleName(rf.role), rf.me, server, rf.nextIndex[server], max(rf.LocalToGlobal(1), rf.nextIndex[server]))
	rf.nextIndex[server] = max(rf.LocalToGlobal(1), rf.nextIndex[server])
}

func (rf *Raftserver) sendBeatCh(server int32, args *rrpc.AppendEntriesArgs, ch chan bool) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := rf.peers[server].AppendEntries(ctx, args)
	if err != nil {
		...
		mylog.DPrintf("%v Send SingleHeartBeat to %v Get Reply:(Success: %v)", rf.me, server, r.Success)
		ch <- r.Success
	}
}
...
func (rf *Raftserver) SingleHeartBeat() bool {
	...
	mylog.DPrintf("Send SingleHeartBeat...")
	chs := make(map[int32]chan bool, len(rf.peers))
	// for i := 0; i < len(rf.peers); i++ {
	for i := range rf.peers {
	...
	mylog.DPrintf("Count: %v, Confirm Leader Situation? %v", count, confirm)
	return confirm
}

func (rf *Raftserver) StartSendAppendEntries() {
	// mylog.DPrintf("server_%v tries to send heartbeat\n", rf.me)
	for !rf.killed() {
		// mylog.DPrintf("L %v StartSendAppendEntries try get Lock\n", rf.me)
		<-rf.HeartbeatTimer.C
		rf.mu.Lock()
		if rf.role != Leader {
			rf.mu.Unlock()
		...
		// mylog.DPrintf("L %v StartSendAppendEntries get the Lock\n", rf.me)
		for i := range rf.peers {
			// for i := 0; i < len(rf.peers); i++ {
			// if i == int(rf.me) {
			// 	continue
			// }
			// mylog.DPrintf("Prepare Send AppendEntries, nextIdx: %v, lastIncludedIndex: %v, global :%v", rf.nextIndex[i], rf.lastIncludedIndex, rf.GlobalToLocal(rf.nextIndex[i]-1))
			args := &rrpc.AppendEntriesArgs{
				Term:         rf.currentTerm,
				LeaderId:     rf.me,
				PrevLogIndex: rf.nextIndex[i] - 1,
			...
			if args.PrevLogIndex < rf.lastIncludedIndex {
				mylog.DPrintf("InstallSnapshot L %v(LastIncludedIdx: %v) >>> %v(PrevLogIdx: %v,NextIdx: %v) ", rf.me, rf.lastIncludedIndex, i, args.PrevLogIndex, rf.nextIndex[i])
		...
		rf.mu.Unlock()
		// mylog.DPrintf("%v %v UnLock in StartSendAppendEntries()", roleName(rf.role), rf.me)
		rf.ResetHeartTimer(HeartbeatTime)
	}
	if rf.killed() {
		mylog.DPrintf("%v %v Killed", roleName(rf.role), rf.me)
	} else {
		mylog.DPrintf("L %v Becomes %v", rf.me, roleName(rf.role))
	}
...
// the struct itself.
func (rf *Raftserver) sendRequestVote(server int32, args *rrpc.RequestVoteArgs) (*rrpc.RequestVoteReply, bool) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := rf.peers[server].RequestVote(ctx, args)
	if err != nil {
		mylog.DPrintf("C %v RequestVote to U %v Err: %v", rf.me, server, err)
		return r, false
	}
	return r, true
}
...
func (rf *Raftserver) Start(command *rrpc.Op) (int64, int64, bool) {
	// mylog.DPrintf("Get %v(Leader: %v) in Start before Lock", rf.me, rf.role == Leader)
	rf.mu.Lock()
	...
	// Your code here (3B).
	if rf.role != Leader {
		mylog.DPrintf("S %v is not Leader", rf.me)
		return -1, -1, false
	}
	...
	rf.persist()
	mylog.DPrintf("Start %v Cmd %v, LastIdx %v Term: %v", rf.me, command, rf.LocalToGlobal(int64(len(rf.log)-1)), rf.currentTerm)
...
func (rf *Raftserver) procVoteAnswer(server int32, args *rrpc.RequestVoteArgs) bool {
	// sendArgs := &args
	...
	rf.mu.Lock()
	mylog.DPrintf("%v %v Vote Result: %v: %v", roleName(rf.role), rf.me, server, reply.VoteGranted)
	defer rf.mu.Unlock()

	if rf.role != Candidate || args.Term != rf.currentTerm {
		// 发送投票请求时，有其他Leader产生了，并通过心跳改变了自己的任期，需要放弃投票
		// mylog.DPrintf("%v 发送投票Term: %v，Term(T: %v)改变，return false", rf.me, args.Term, rf.currentTerm)
		return false
	}
...
func (rf *Raftserver) collectVote(server int32, args *rrpc.RequestVoteArgs) {
	ok := rf.procVoteAnswer(server, args)
	if !ok {
		// mylog.DPrintf("%v Votes Return False", rf.me)
		return
	}
	rf.mu.Lock()
	if rf.voteCount > int32(len(rf.peers))/2 {
		// 如果投票数已经超过了半数，直接返回，因为之前的协程已经处理过了
		// mylog.DPrintf("%v skip Check vote", rf.me)
		rf.mu.Unlock()
		return
	}
	rf.voteCount += 1
	// mylog.DPrintf("%v(T: %v) Try Check vote", rf.me, rf.currentTerm)
	...
	if rf.voteCount > int32(len(rf.peers))/2 && rf.role == Candidate && !outTime {
		...
		// 需要设置 nextIndex 等
		var i int32
		for i = 0; i < int32(len(rf.nextIndex)); i++ {
			// rf.nextIndex[i] = len(rf.log)
			// rf.matchIndex[i] = 0
			rf.nextIndex[i] = rf.LocalToGlobal(int64(len(rf.log)))
			rf.matchIndex[i] = rf.lastIncludedIndex
		}
		mylog.DPrintf("C %v(T: %v,LastLogI: %v,LastLogT: %v,LastLog: %v,CommitI: %v) becomes new Leader", rf.me, rf.currentTerm, len(rf.log)-1, rf.log[len(rf.log)-1].Term, rf.log[len(rf.log)-1].Command, rf.commitIndex)
		// rf.mu.Unlock()
		// 发送心跳消息或复制消息
		rf.ResetHeartTimer(ImmidiateTime)
...
func (rf *Raftserver) StartElection() {
	...
	rf.votedFor = rf.me
	rf.persist()
	// mylog.DPrintf("%v %v(T: %v) start RequestVote\n", roleName(rf.role), rf.me, rf.currentTerm)
	rf.voteCount = 1
...
func (rf *Raftserver) ticker() {
	for !rf.killed() {
		...
		// mylog.DPrintf("%v server_%v checking itself\n", roleName(rf.role), rf.me)
		// <-rf.HeartbeatTimer.C
		rf.mu.Lock()
...
func (rf *Raftserver) connectToPeers() error {
	...
	for i, addr := range rf.peerAddrs {
		...
		mylog.DPrintf("Node %v Try connect to %v: %s", rf.me, i, addr)
		conn, err := grpc.NewClient(addr, grpc.WithTransportCredentials(insecure.NewCredentials()))
		if err != nil {
			mylog.DPrintf("did not connect to %s: %v", addr, err)
			failed = append(failed, i)
			continue
		}
		if conn == nil {
			mylog.DPrintf("No server at %s", addr)
			return fmt.Errorf("no server listening at %s", addr)
		}
		rf.peers[i] = rrpc.NewRaftClient(conn)
	}
	maxRetry := 30
	for len(failed) > 0 && maxRetry > 0 {
	...
	if maxRetry == 0 {
		return fmt.Errorf("raft node %d retry connect to others failed after 30 times", rf.me)
	}
	return nil
}

func (rf *Raftserver) serve() {
	lis, err := net.Listen("tcp", rf.addr)
	if err != nil {
		mylog.DPrintf("node listen failed at %s, error: %v", rf.addr, err)
		return
	}
	s := grpc.NewServer()
	rrpc.RegisterRaftServer(s, rf)
	mylog.DPrintf("node %v listening at %v", rf.me, lis.Addr())
...
func (rf *Raftserver) startWork() {
	...
	mylog.DPrintf("Node %v Has %v peers: %v", rf.me, len(rf.peers), rf.peers)
	rf.QuickApply()
	go rf.ticker()
	go rf.CommitCheck()
}
...
func Make(peerAddrs map[int32]string, me int32,
	persister *persister.Persister, applyCh chan ApplyMsg, addr string, prepareCh chan struct{}) *Raftserver {
	rf := &Raftserver{
		addr:      addr,
		peerAddrs: peerAddrs,
		peers:     make(map[int32]rrpc.RaftClient),
		me:        me,
		persister: persister,
		applyCh:   applyCh,
		dead:      0,
		...
		currentTerm: 0,
		votedFor:    -1,
		nextIndex:   make(map[int32]int64, len(peerAddrs)),
		matchIndex:  make(map[int32]int64, len(peerAddrs)),
		commitIndex: 0,
		lastApplied: 0,
		role:        Follower,
	...
	mylog.DPrintf("Init StateMachine %v", me)
	go func() {
		go rf.serve()
		time.Sleep(1 * time.Second)
