...
func (l *Log) writeBatch(b *Batch) error {
	...
	for i := 0; i < len(b.entries); i++ {
		if b.entries[i].index != l.lastIndex+uint64(i+1) {
			...
			return fmt.Errorf("out of order, need: %v but get: %v", l.lastIndex+uint64(i+1), b.entries[i].index)
		}
	}
...
func (l *Log) cycle() error {
	...
	var err error
	l.sfile, err = os.OpenFile(s.path, os.O_CREATE|os.O_RDWR|os.O_TRUNC, l.opts.DirPerms)
	if err != nil {
		return err
	}
...
func (l *Log) loadSegment(index uint64) (*segment, error) {
	...
	var rseg *segment
	l.scache.Range(func(_, value interface{}) bool {
		s := value.(*segment)
